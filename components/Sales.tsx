...
export const Sales: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    const enrichedOrders = orderList.map((order) => ({
      ...order,
      customerName: customerList.find((customer) => customer.id === order.customerId)?.name || 'Cliente Removido',
      productName: productList.find((product) => product.id === order.productId)?.name || 'Produto Removido',
    }));
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    result.sort((left, right) => {
      const leftValue = left[sortConfig.field] || 0;
      const rightValue = right[sortConfig.field] || 0;
  ...
  const inputClasses =
    'w-full rounded-2xl border-slate-200 bg-white p-3 border text-slate-900 font-medium focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all text-sm appearance-none';
    ...
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
      ...
      {showForm && (
        <form
          ...
          className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-2xl space-y-5 max-w-3xl mx-auto animate-slideDown"
        >
              ...
              >
                ...
                {products.map((product) => (
                  <option key={product.id} value={product.id} disabled={product.stockCount <= 0}>
                    {product.name} (R$ {product.sellingPrice.toFixed(2)}) - {product.stockCount} em estoque
                  </option>
                ))}
              </select>
            </div>
      ...
      <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-5 items-center">
        <div className="w-full md:w-auto flex-1 grid grid-cols-2 md:flex md:flex-row gap-4">
          <div className="relative">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest absolute -top-2 left-3 bg-white px-1 z-10">
              Status
            </label>
            <select
              value={filterStatus}
        ...
        <div className="hidden md:block">
          <p className="text-xs font-bold text-slate-400 whitespace-nowrap">
            <span className="text-slate-900 font-black">{filteredAndSortedOrders.length}</span>{' '}
            resultados
          </p>
        </div>
      </div>
        ...
        {filteredAndSortedOrders.map((order) => (
          <div key={order.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-50">
              <div className="text-xl font-black text-slate-950">R$ {order.totalAmount.toFixed(2)}</div>
                ...
                {order.status === SaleStatus.PENDING && (
                  <>
                    <button
                      ...
                      disabled={loading}
                      className="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center active:scale-90 transition-transform shadow-sm disabled:opacity-50"
                    >
                      {loading ? (
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="w-10 h-10 bg-rose-50 text-rose-600 rounded-2xl flex items-center justify-center active:scale-90 transition-transform shadow-sm disabled:opacity-50"
                    >
                      <XIcon className="w-4 h-4" />
                    </button>
                  </>
      ...
      {filteredAndSortedOrders.length === 0 && (
        <div className="py-24 text-center bg-white rounded-[2rem] border border-dashed border-slate-200">
          <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <SearchIcon className="w-9 h-9 text-slate-300" />
          </div>
